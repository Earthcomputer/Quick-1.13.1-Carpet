...
public class SpawnReporter
{
    ...
    {
        String name = world.dimension.getType().toString();
    ...
    public static List<ITextComponent> report(BlockPos pos, World worldIn)
    {
        ...
        int lc = chunk.getTopBlockY(Heightmap.Type.LIGHT_BLOCKING, x, z) + 2;
        ...
        for (EnumCreatureType enumcreaturetype : EnumCreatureType.values())
        {
            ...
            if (lst != null && !lst.isEmpty())
            {
                for (Biome.SpawnListEntry animal : lst)
                {
                    ...
                    try
                    {
                        entityliving = (EntityLiving)animal.entityType.create(worldIn);
                    }
                    ...
                    String creature_name = EntityType.getId(entityliving.getType()).toString();
                    String pack_size = String.format("%d-%d", animal.minGroupCount, animal.maxGroupCount);
